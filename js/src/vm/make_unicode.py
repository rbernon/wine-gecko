...
ZWNJ = ord('\N{ZERO WIDTH NON-JOINER}')
ZWJ = ord('\N{ZERO WIDTH JOINER}')
...
""")
    ...
    test_space.write('var onlySpace = String.fromCharCode(' +
                     ', '.join([hex(c) for c in test_space_table]) + ');\n')
    test_space.write("""
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        reader = urllib.request.urlopen('http://unicode.org/Public/UNIDATA/UnicodeData.txt')
        data = reader.read()
        reader.close()
    ...
    if len(sys.argv) > 2:
        ...
        reader = urllib.request.urlopen('http://unicode.org/Public/UNIDATA/CaseFolding.txt')
        data = reader.read()
        reader.close()
