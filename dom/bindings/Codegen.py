...
def toBindingNamespace(arg):
    if arg.endswith('_workers'):
        return arg[0:-8]+'Binding_workers'
    else:
        return arg+'Binding'
...
class CGHeaders(CGWrapper):
    ...
    def __init__(self, descriptors, dictionaries, callbacks,
        ...
        def addHeadersForType(xxx_todo_changeme):
            """
            ...
            """
            (t, descriptor, dictionary) = xxx_todo_changeme
...
def getJSToNativeConversionInfo(type, descriptorProvider, failureCode=None,
    ...
    if type.isUnion():
        ...
        arrayObjectMemberTypes = [t for t in memberTypes if t.isArray() or t.isSequence()]
        ...
        mozMapMemberTypes = [t for t in memberTypes if t.isMozMap()]
    ...
    def __init__(self, nativeMethodName, static, descriptor, method,
        ...
        def filteredSignatures(signatures, descriptor):
            ...
            if descriptor.workers:
                ...
                signatures = [sig for sig in signatures if all(typeExposedInWorkers(arg.type)
                                    for arg in sig[1])]
                if len(signatures) == 0:
...
class CGRegisterWorkerBindings(CGAbstractMethod):
    ...
    def definition_body(self):
        ...
        workerDescriptorIfaceNames = set(d.interface.identifier.name for
                                         d in descriptors)
        descriptors.extend(
            [d for d in self.config.getDescriptors(hasInterfaceObject=True,
                                           isExposedInAnyWorker=True,
                                           register=True,
                                           skipGen=False,
                                           workers=False) if d.interface.identifier.name not in workerDescriptorIfaceNames])
        conditions = []
        for desc in descriptors:
